...
class CHILDESCorpusReader(XMLCorpusReader):
    ...
    def words(self, fileids=None, stem=False,
        ...
        pos=False
        if not self._lazy:
            return [self._get_words(fileid, sent, stem, relation,
                pos, strip_space, replace) for fileid in self.abspaths(fileids)]

        get_words = lambda fileid: self._get_words(fileid, sent, stem, relation,
            pos, strip_space, replace)
        return LazyConcatenation(LazyMap(get_words, self.abspaths(fileids)))

    def tagged_words(self, fileids=None, stem=False,
            relation=False, strip_space=True, replace=False):
        """
    ...
    def sents(self, fileids=None, stem=False,
    ...
    def tagged_sents(self, fileids=None, stem=False,
            relation=None, strip_space=True, replace=False):
        """
    ...
    def _get_words(self, fileid, sent, stem, relation, pos,
            strip_space, replace):
        xmldoc = ElementTree.parse(fileid).getroot()
